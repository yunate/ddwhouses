...
bool ddmain_cli::run_in_helper_process(const std::wstring& target, const std::wstring& hepler_exe_path)
{
    ...
    auto cmd = ddstr::format("-helper %s %s", ddstr::utf16_8(target).c_str(), ddstr::utf16_8(out_file_path).c_str());
    auto helper_porcess = ddsub_process::create_instance(ddstr::utf16_8(hepler_exe_path), cmd);
    s32 wait_times = 6;
